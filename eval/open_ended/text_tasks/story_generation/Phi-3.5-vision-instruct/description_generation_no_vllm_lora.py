...
def run_inference(model, processor, question: str) -> str:
    """
    使用给定的模型和输入进行推理。
...
def process_dataset(dataset_path, field_name, elements, sample_nums, model_name, model_path, random_seed=None, custom_suffix=""):
    """
    ...
    for entry in tqdm(sampled_ds, desc=f"Processing sampled dataset: {dataset_path}"):
        try:
            ...
            # 运行推理
            response = run_inference(model, processor, question)
